...
def fplot(fvals, f_data, ref, alt, year=2017, nbins=130, savename=None, mc=False):
    ...
    print(mc,year,ax)
    #hep.cms.label(data=not mc, year=int(year), ax=ax)
    ax.set_xlim(0, x_lim);
...
if __name__ == "__main__":
    ...
    #print("names,names_alt",names,names_alt)
    ...
    #print("skim_gofs",list(skim_gofs([os.path.join(dirname, 'higgsCombine.Base.GoodnessOfFit.mH120.root')]).values())[0])
    data_ref = list(skim_gofs([os.path.join(dirname, 'higgsCombine.Base.GoodnessOfFit.mH120.root')]).values())[0]
    data_alt = list(skim_gofs([os.path.join(dirname, 'higgsCombine.Alt.GoodnessOfFit.mH120.root')]).values())[0]
